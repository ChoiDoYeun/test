...
def create_gui(warehouse, callback=None):
    ...
    warehouse_frame.place(x=10, y=10, width=270, height=220)
    ...
    for i, building in enumerate(['A', 'B', 'C', 'D']):
        ...
        label = tk.Label(frame, text=building, font=("Arial", 12), width=5, height=1)
    ...
    choose_frame.place(x=290, y=10, width=250, height=220)     
    ...
    for i in range(6):
        for j in range(4):
            ...
            button = tk.Button(choose_frame, text=button_text, width=5, height=2, command=lambda bt=button_text: callback(bt, root, warehouse, warehouse_labels) if callback else None)
            button.grid(row=i, column=j, padx=2, pady=2)

    # 모드 선택 섹션
    mode_frame = tk.Frame(root)
    mode_frame.place(x=550, y=10, width=240, height=220)
    
    input_on_button = tk.Button(mode_frame, text="INPUT ON", width=10, height=2, command=lambda: input_on(warehouse))
    input_on_button.pack(padx=5, pady=5)

    input_off_button = tk.Button(mode_frame, text="INPUT OFF", width=10, height=2, command=input_off)
    input_off_button.pack(padx=5, pady=5)
    
    output_on_button = tk.Button(mode_frame, text="OUTPUT ON", width=10, height=2, command=lambda: output_on(warehouse))
    output_on_button.pack(padx=5, pady=5)

    output_off_button = tk.Button(mode_frame, text="OUTPUT OFF", width=10, height=2, command=output_off)
    output_off_button.pack(padx=5, pady=5)

    # 실행 및 중지 버튼 섹션
    control_frame = tk.Frame(root)
    control_frame.place(x=290, y=240, width=250, height=100)

    tk.Button(control_frame, text="RUN", bg="green", command=lambda: run(warehouse), width=10, height=2).pack(side=tk.LEFT, padx=5, pady=5)
    tk.Button(control_frame, text="STOP", bg="red", command=stop, width=10, height=2).pack(side=tk.RIGHT, padx=5, pady=5)
